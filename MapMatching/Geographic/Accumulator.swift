...
public struct Accumulator {
    ...
    /// - parameter a: set <i>sum</i> = <i>a</i>.
    public init(_ a: Accumulator) {
        _s = a._s
        _t = a._t
    }
    ...
    /// - parameter y: set <i>sum</i> = <i>y</i>.
    public mutating func set(_ y: Double) {
        _s = y
        _t = 0
    }
    ...
    /// - parameter y: the number to be added to the sum.
    /// - returns: <i>sum</i> + <i>y</i>.
    public func sum(_ y: Double)-> Double {
    ...
    /// - parameter y: Set <i>sum</i> += <i>y</i>.</param>
    public mutating func add(_ y: Double) {
        // Here's Shewchuk's solution...
        // hold exact sum as [s, t, u]
        // Accumulate starting at least significant end
        let r1 = GeoMath.sum(y, _t);
        let y1 = r1.0
        let u = r1.1
        
        let r2 = GeoMath.sum(y1, _s);
        _s = r2.0
        _t = r2.1
